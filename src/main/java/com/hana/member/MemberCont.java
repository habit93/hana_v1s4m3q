...
public class MemberCont {
  @Autowired
  @Qualifier("com.hana.member.MemberDAO")
  ...
  @RequestMapping(value = "/member/admin.do", method = RequestMethod.GET)
  public ModelAndView passwd(int mno) {
    ModelAndView mav = new ModelAndView();
    mav.setViewName("/member/admin"); // /webapp/member/passwd.jsp

    mav.addObject("mno", mno);
    return mav;
  }
  
  @RequestMapping(value = "/member/admin.do", method = RequestMethod.POST)
  ...
  public ModelAndView login(MemberVO memberVO, 
    ...
    if (memberDAO.login(memberVO) == 1) {
      // id를 이용하여 회원 정보를읽어오는 코드의 추가 
      MemberVO vo = memberDAO.mno(memberVO.getId());
